...
export function Sidebar() {
  ...
  return <aside className="fixed inset-y-0 left-0 z-10 flex w-64 flex-col border-r border-border bg-surface px-5 py-6">
    ...
    <nav className="space-y-1">{navigation.map((item, index) => { const active = item === "Overview" ? pathname === "/" : pathname === paths[item] || pathname.startsWith(`${paths[item]}/`); return <Link key={item} href={paths[item]} className={`flex items-center gap-3 rounded-md px-3 py-2.5 text-sm ${active ? "bg-elevated text-paper" : "text-muted hover:bg-elevated hover:text-paper"}`}><span className="w-4 text-center text-xs text-quiet">{["⌂", "◌", "↗", "◇", "◉", "!", "≡", "⚙", "·"][index]}</span>{item}</Link>; })}</nav>
