...
export async function executeRecoveryAction(input: ExecuteActionInput): Promise<ActionExecutionResult> {
  ...
  try {
    ...
    else if (executableAction === "ESCALATE") { transitionCase(current.status, "ESCALATED"); await input.repository.updateCase(input.merchantId, input.caseId, { status: "ESCALATED", escalated: true }); }
